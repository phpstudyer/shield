 ...
 * @LastEditTime: 2023-08-15 12:18:45
 * @FilePath: /src/shield/util/utils.go
 */
package util
...
func TimingSync(encrypt *Encrypt, machineID string, d time.Duration, ch chan os.Signal) {
	defer close(ch)
	...
	for range ticker.C {
		...
		if encrypt.EndAt.Before(encrypt.SyncedAt) {
			return
		}
	}
}
...
func Run(appSymbol string, obj Runable) {
	...
	_, err := startVerify(appSymbol, ch)
	if err != nil {
...
func startVerify(appSymbol string, ch chan os.Signal) (*Encrypt, error) {
